...
class DoctorPage {
  ...
  void endoscope() {
    ...
    for (int y = 0; y < bodyCover.height; y++ ) {
      for (int x = 0; x < bodyCover.width; x++ ) {
        ...
        //make a circle from the distance and see it inside
        if (distance < 70) {
  ...
  void x_ray() {
    ...
    for (int y = 0; y < boneBody.height; y++ ) {
      for (int x = 0; x < boneBody.width; x++ ) {
        ...
        //make a circle and see it inside
        if (distance < 70) {
  ...
  void goHeartRoom() {
    ...
    //make a stethoscope with mouse movement
    stroke(0);
    strokeWeight(5);
    fill(220);
    ellipseMode(CENTER);
    ...
    //draw heartRate with oscillation
    stroke(255, 0, 0);
    noFill();
    beginShape();
    ...
    //express heart rate as a number
  ...
  void goMedicineRoom() {
    ...
    //If you submit it, you will prescribe it according to the checked item
    if (submit) {
