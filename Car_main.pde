import ddf.minim.*;
Minim minim;
AudioPlayer song, crash, boost, ghostsound, handling;
boolean over;
boolean ghostMode;
int ghostv=0;
PImage tree;
...
void setup() {
  ...
  cars.add(new Car(myCar, width/2, height/2+140, 0.03));
  for (int i=ecars.size()-1; i>0; i--) ecars.remove(i);
...
void draw() {
  ...
  if (over==true) {
    ...
    text("press R: Replay", width/2, height/2+40);
    text("press H: Home", width/2-5, height/2+90);
  }
  theta+=0.05;
  if (ghostMode==true) {
    ghostv=millis();
